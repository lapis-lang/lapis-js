/**
 * Tests for query() — the coalgebraic dual of relation().
 *
 * query() wraps behavior() with cospan structure (output/done/accept fold ops)
...
import { query, behavior } from '../index.mjs';
...
/** Graph reachability as query (dual perspective of relation closure). */
...
describe('query()', () => {
    ...
    describe('explore()', () => {
        it('should exist on the query ADT', () => {
            const Counter = makeCounter();
    ...
    describe('duality with relation', () => {
        it('explore is dual to closure: both compute reachability', () => {
            // query().explore() computes reachability top-down (lazy, first path)
