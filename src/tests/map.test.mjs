...
describe('Map Operations', () => {
    ...
    describe('Recursive Structures', () => {
        ...
        test('Nested tree structure preserves shape', () => {
            const Tree = data(({ Family, T }) => ({
                ...
                .map('square', (Family) => ({ out: Family }), { T: (x) => x * x });
    ...
    describe('Multiple Type Parameters', () => {
        test('Pair transforms both type parameters independently', () => {
            const Pair = data(({ T, U }) => ({
                ...
                .map('transform', (Family) => ({ out: Family }), {
                    T: (x) => x * 2,
                    U: (s) => s.toUpperCase()
                });
        ...
        test('Either transforms Left and Right with same type', () => {
            const Either = data(({ T }) => ({
                ...
                .map('negate', (Family) => ({ out: Family }), { T: (x) => -x });
        ...
        test('Heterogeneous Either with different transforms', () => {
            const Either = data(({ T, U }) => ({
                Left: { value: T },
                Right: { value: U }
            }))
                .map('convert', (Family) => ({ out: Family }), {
                    T: (x) => String(x),
                    U: (s) => s.length
                });
    ...
    describe('Callback Form', () => {
        test('supports callback form for transforms', () => {
            const List = data(({ Family, T }) => ({
                ...
                .map('double', (Family) => ({ out: Family }), (Family) => ({
                    T: (x) => x * 2
                }));
    ...
    describe('Error Handling', () => {
        test('throws on non-camelCase operation name', () => {
            ...
            assert.throws(
                () => List.map('Increment', { out: List }, { T: (x) => x + 1 }),
        ...
        test('handles missing transform gracefully', () => {
            const Pair = data(({ T, U }) => ({
                ...
                // Only transform T, not U
                .map('transformFirst', (Family) => ({ out: Family }), {
                    T: (x) => x * 2
                    // U transform missing
                });
        ...
        test('supports multiple arguments in transform functions', () => {
            const List = data(({ Family, T }) => ({
                ...
                .map('combine', (Family) => ({ out: Family }), { T: (x, a, b) => x + a + b });
