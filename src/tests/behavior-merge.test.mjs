...
function makeStream() {
    return behavior(({ Self, T }) => ({
        ...
        negated: { [op]: 'map', T: (x) => -x },
        // Static merge: unfold + map + fold  → PascalCase
        TakeFrom: {
            [op]: 'merge',
            [operations]: ['From', 'take']
        },
        TakeDoubled: {
            [op]: 'merge',
        ...
        doubledTake: {
            [op]: 'merge',
            [operations]: ['doubled', 'take']
        },
        doubledNegatedTake: {
            [op]: 'merge',
            [operations]: ['doubled', 'negated', 'take']
        }
    }));
}
    ...
    it('instance merge (no unfold) must be camelCase', () => {
        assert.throws(() => {
            behavior(({ Self, T }) => ({
                ...
                doubled: { [op]: 'map', T: (x) => x * 2 },
                DoubledTake: {                      // should be camelCase
                    [op]: 'merge',
                    [operations]: ['doubled', 'take']
                }
            }));
