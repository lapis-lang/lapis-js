...
describe('Predicates', () => {
    ...
    test('throws error when predicate fails', () => {
        ...
        assert.throws(
            () => EvenPoint.Point2({ x: 3, y: 4 } as any),
    ...
    test('supports custom predicates for strings', () => {
        ...
        assert.throws(
            () => User.RegisteredUser({ email: 'invalid', username: 'testuser' } as any),
    ...
    test('supports predicates with ranges', () => {
        ...
        assert.throws(
            () => Percentage.Value({ amount: 150 } as any),
    ...
    test('supports mixing predicates and constructors', () => {
        ...
        assert.throws(
            () => Product.Item({ name: 'Widget', price: -5, quantity: 1 } as any),
    ...
    test('supports predicates with ADT fields', () => {
        ...
        assert.throws(
            () => ColorPoint.EvenPoint({ x: 3, y: 4, color: Color.Red } as any),
            /Field 'x' failed predicate validation/
        );
    });

    test('supports complex predicate logic', () => {
        const isPrime = (x: unknown): x is number => {
