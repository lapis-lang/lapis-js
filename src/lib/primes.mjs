 ...
 * Imported by examples (../src/lib/primes.mjs) and tests (../lib/primes.mjs).
 */
...
const Divisors = data(({ Family }) => ({
    ...
    hasFactor: {
        ...
        Nil() { return false; },
        Cons({ head, tail }, n) { return n % head === 0 || tail(n); }
    },
    firstFactor: {
        [op]: 'fold',
        [spec]: { in: Number, out: Number },
        Nil() { return 0; },
        Cons({ head, tail }, n) { return n % head === 0 ? head : tail(n); }
    },
    OddRange: {
        [op]: 'unfold',
        [spec]: { in: { start: Number, limit: Number }, out: Family },
        Nil: ({ start, limit }) => start > limit ? {} : null,
        Cons: ({ start, limit }) =>
