...
// Internal registry of open readable streams, keyed by handle string.
// Handles are generated by OpenStream and consumed by ReadChunk / CloseStream.
const openStreams = new Map<string, ReadableStreamDefaultReader<Uint8Array>>();
let streamCounter = 0;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function executeRequest(request: any): Promise<any> {
    ...
    if (request instanceof Req.OpenStream) {
        ...
        const handle = `stream-${++streamCounter}`;
        openStreams.set(handle, response.body.getReader());

        return IOResponse.StreamOpened({ handle });
    }

    if (request instanceof Req.ReadChunk) {
        const reader = openStreams.get(request.handle);
        ...
        if (done) {
            openStreams.delete(request.handle);
